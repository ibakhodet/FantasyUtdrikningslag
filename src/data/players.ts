...
// Spillere som har meldt avbud. Filtreres ut av lag, autofyll og tabeller.
export const WITHDRAWN: Person[] = [
  { id: 'erik', name: 'Erik', nick: 'Skallen', color: '#7a5640', photo: 'avatars/erik.png', bio: '' },
];
export const WITHDRAWN_IDS = new Set<string>(WITHDRAWN.map((p) => p.id));
...
export const PEOPLE_BY_ID: Record<string, Person> = Object.fromEntries(
  [...PLAYERS, ...WITHDRAWN, ...GUESTS].map((p) => [p.id, p]),
);
